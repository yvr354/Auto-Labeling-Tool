...
const DataAugmentation = ({ datasetId, onClose }) => {
  ...
  const loadPresets = async () => {
    try {
      const response = await fetch('/api/augmentation/presets');
  ...
  const loadDefaultConfig = async () => {
    try {
      const response = await fetch('/api/augmentation/default-config');
      const data = await response.json();
  ...
  const loadAugmentationJobs = async () => {
    try {
      const response = await fetch(`/api/augmentation/jobs/${datasetId}`);
      const data = await response.json();
  ...
  const handlePreview = async () => {
    try {
      ...
      const response = await fetch('/api/augmentation/preview', {
        method: 'POST',
  ...
  const handleCreateAugmentation = async () => {
    try {
      ...
      const response = await fetch('/api/augmentation/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          dataset_id: datasetId,
  ...
  const handleDeleteJob = async (jobId) => {
    try {
      const response = await fetch(`/api/augmentation/job/${jobId}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        loadAugmentationJobs();
